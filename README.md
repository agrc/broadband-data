...
The general idea is that we take the BSL location IDs from the FCC data and aggregate them to the H3 hexes. Due to CostQuest licensing, we can't make the fabric public, so we need to aggregate. We then show the best service available in each hex. The three hex level layers group the hexes by provider, technology, and speed and then dissolve the hexes together so that you can identify what speed and tech is available from what provider in any given area. It's not super precise. See https://gis.utah.gov/blog/2025-11-04-new-broadband-service-data/ for more info about the process.
